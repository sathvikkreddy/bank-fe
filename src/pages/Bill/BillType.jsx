...
const BillType = () => {
    ...
    const fetchBankAccounts = async () => {
      try {
        ...
        const response = await axios.get(
          "https://techbuzzers.somee.com/GetBankAccounts",
          {
            headers: {
  ...
  const fetchBillDetails = async (billType) => {
    try {
      ...
      if (!token) throw new Error("Unauthorized: Token not found");
      const response = await axios.get(
        "https://techbuzzers.somee.com/getBillDetails",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const billDetails = response.data.find(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://techbuzzers.somee.com/payBill",
        {
  ...
  const fetchRecentPayments = async () => {
    try {
      ...
      if (!token) throw new Error("Unauthorized: Token not found");
      const response = await axios.get(
        "https://techbuzzers.somee.com/getRecentBillPayments",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
              ...
              <Card title={`Bill Details Form for ${selectedBillType}`}>
                <form onSubmit={handleSubmit} className="mt-8">
                  <div className="mb-4">
                    <label
                      htmlFor="billId"
                      className="font-semibold mb-2 hidden"
                    >
                      Bill ID
                    </label>
                    <input
